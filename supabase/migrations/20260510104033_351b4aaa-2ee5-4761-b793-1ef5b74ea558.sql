...
CREATE TABLE public.sub_user_pool (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  suname text NOT NULL UNIQUE,
  passwd text NOT NULL,
  host text NOT NULL DEFAULT 'global.rotgb.711proxy.com',
  port text NOT NULL DEFAULT '10000',
  proto text NOT NULL DEFAULT 'http',
  mb_capacity numeric NOT NULL,
  expire_at timestamptz,
  assigned_to_order_id uuid REFERENCES public.proxy_orders(id) ON DELETE SET NULL,
  assigned_at timestamptz,
  note text,
  created_at timestamptz NOT NULL DEFAULT now()
);

ALTER TABLE public.sub_user_pool ENABLE ROW LEVEL SECURITY;

CREATE POLICY "admins manage sub_user_pool" ON public.sub_user_pool
  FOR ALL USING (public.has_role(auth.uid(), 'admin'::public.app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin'::public.app_role));

CREATE INDEX idx_subuser_pool_available ON public.sub_user_pool (mb_capacity)
  WHERE assigned_to_order_id IS NULL;
...
BEGIN
  ...
  required_mb := o.gb_amount * 1024;

  -- Pick smallest pool entry that fits (best-fit), lock it
